...
function Dashboard({ session, onSessionUpdate, onLogout }: { session: Session; onSessionUpdate: (session: Session) => void; onLogout: () => void }) {
  ...
  const deviceConnected = Boolean(device?.assigned && device.connected);
  const monitoredBattery = deviceConnected && device?.state_of_charge != null
  ...
  const refreshDevice = useCallback(async () => {
    ...
    const patientQuery = session.user.role === "doctor"
      ? `?patient_id=${encodeURIComponent(selectedPatient!.id)}`
      : "";
    try {
      setDevice(await api<DeviceStatus>(`/api/v1/device/status${patientQuery}`, {}, session.access_token));
    } catch {
      setDevice(null);
    }
  }, [selectedPatient, session.access_token, session.user.role]);
  ...
  return (
    <SafeAreaView edges={["top", "left", "right"]} style={[styles.dashboardSafe, dark && styles.screenDark]}>
      ...
      <ScrollView style={dark && styles.screenDark} contentContainerStyle={styles.dashboardContent}>
        ...
        {session.user.role === "patient" && <PatientDeviceSummary device={device} battery={monitoredBattery} monitoring={monitoringTab === "night" ? Boolean(nightStatus?.active) : dayMonitoringActive} />}
        ...
        {session.user.role === "doctor" && !selectedPatient ? (
          ...
          <>
            ...
            {session.user.role === "doctor" && <PatientDeviceSummary device={device} battery={monitoredBattery} monitoring={monitoringTab === "night" ? Boolean(nightStatus?.active) : dayMonitoringActive} />}
...
function NightModePanel({ status, sample, clock, statusSyncedAt, positionSince, busy, error, onToggle, readOnly = false }: { status: NightStatus | null; sample: NightSample | null; clock: number; statusSyncedAt: number; positionSince: number; busy: boolean; error: string; onToggle: () => void; readOnly?: boolean }) {
  ...
  return (
    <View style={[styles.nightCard, active && styles.nightCardActive, dark && styles.nightCardDark]}>
      {active ? (
        <>
          {!readOnly && <Pressable disabled={busy || !status} onPress={onToggle} style={({ pressed }) => [styles.nightButton, styles.nightStopButton, pressed && styles.pressed]}>
            {busy ? <ActivityIndicator color="#fff" /> : <Text style={styles.nightButtonText}>TERMINA MONITORAGGIO</Text>}
          </Pressable>}
      ...
      ) : readOnly ? <Text style={styles.nightDescription}>La modalità notte non è attiva. Il pannello mostrerà automaticamente i dati quando il monitoraggio verrà avviato.</Text> : null}
      {error ? <Text style={styles.nightError}>{error}</Text> : null}
      {!readOnly && !active && <Pressable disabled={busy || !status} onPress={onToggle} style={({ pressed }) => [styles.nightButton, styles.nightStartButton, pressed && styles.pressed]}>
        {busy ? <ActivityIndicator color="#fff" /> : <Text style={styles.nightButtonText}>ATTIVA MONITORAGGIO</Text>}
      </Pressable>}
    </View>
  );
}
...
function PatientDeviceSummary({ device, battery, monitoring }: { device: DeviceStatus | null; battery: number | null; monitoring: boolean }) {
  const { dark } = useAppTheme();
  const connected = Boolean(device?.assigned && device.connected);
  const deviceLabel = connected
    ? device?.display_name ?? "Maglia"
    : "OFF";
  return <View style={[styles.deviceSummary, dark && styles.surfaceDark]}>
    <View style={styles.deviceSummaryItem}><Text style={[styles.deviceSummaryIcon, !connected && styles.deviceSummaryIconOff]}>▣</Text><View style={{ flex: 1 }}><Text style={[styles.deviceSummaryLabel, dark && styles.mutedDark]}>Dispositivo</Text><Text numberOfLines={2} style={[styles.deviceSummaryValue, connected ? (dark && styles.textDark) : styles.monitoringTextOff]}>{deviceLabel}</Text></View></View>
